			...
			<div key={"value-item-" + value.id}>
				{value.required && (
					<Icon.ExclamationCircleFill
						key={"value-item-" + value.id + "-required"}
						className="mr-1"
					/>
				)}
				...
				{value.description && (
					<div>
						<Icon.ChatDots key={"value-item-" + value.id + "-description"} className="mr-1" />
						{value.description}
					</div>
				)}
			</div>
		...
		>
			...
			<Nav as="nav">
				...
				<OverlayTrigger
					...
					rootClose
				>
					<Button type="button" size="sm" variant="danger" className="my-0 px-2 shadow-none">
						<Icon.Trash size="20" />
					</Button>
				</OverlayTrigger>
			</Nav>
	...
	render() {
		...
		return (
			<div className="d-flex flex-column">
				{items.length === 0 ? (
					<InfoBox icon={<Icon.EmojiDizzy size="64" />} content={messageNoValuesDefinitions} />
				) : (
					items.map((value) => (
						<ValueItem
							key={"value-" + value.id}
