...
router.get("/", async (req, res) => {
	try {
		const examinations = await Examination.find({ user: req.user.id })
			.sort({
				group: 1,
				name: 1,
			})
...
router.post("/:id", async (req, res) => {
	...
	try {
		...
		if (processValues) {
			...
			if (valuesToCreate.length > 0) response.values.created = await Value.create(valuesToCreate);
...
router.delete("/", (req, res) => {
	...
	if (!selectedItems || typeof selectedItems !== "object" || !selectedItems instanceof Array) {
...
router.get("/configuration", async (req, res) => {
	try {
		const resource = await Configure.find({ resource: "examination-list-view" }).lean();
